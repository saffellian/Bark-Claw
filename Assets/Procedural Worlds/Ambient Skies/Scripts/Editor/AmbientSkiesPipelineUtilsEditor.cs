    ...
    public class AmbientSkiesPipelineUtilsEditor : EditorWindow
    {
        ...
        private void FinalizeEnvironment()
        {
            switch (m_pipelineToInstall)
            {
                case AmbientSkiesConsts.RenderPipelineSettings.BuiltIn:
                    #region Built-In                                       
                    AmbientSkiesPipelineUtils.SetupPipeline(m_pipelineToInstall, null, "Procedural Worlds/Simple Water", null, null, m_finalizeEnvironment);
                    if (m_showDebug)
                    {
                    ...
                    if (m_finalizeEnvironment)
                    {
                        ...
                        {
                            AmbientSkiesPipelineUtils.SetupHDEnvironmentalVolume(m_selectedSkyboxProfile, m_profiles, m_selectedSkyboxProfileIndex, m_pipelineToInstall, "High Definition Environment Volume", "Ambient Skies HD Volume Profile");
                        }
                        else
                        {
                            if (m_showDebug)
                ...
                case AmbientSkiesConsts.RenderPipelineSettings.Lightweight:
                    #region Lightweight                    
                    AmbientSkiesPipelineUtils.SetupPipeline(m_pipelineToInstall, "Procedural Worlds Lightweight Pipeline Profile Ambient Skies", "Procedural Worlds/Simple Water LW", "Pipeline Terrain Material", "Lightweight Render Pipeline/Terrain/Lit", m_finalizeEnvironment);
                    if (m_showDebug)
                    {
                        Debug.Log(m_profiles);
                        Debug.Log(m_selectedSkyboxProfile);
                ...
                case AmbientSkiesConsts.RenderPipelineSettings.HighDefinition:
                    #region High Definition    
#if !UNITY_2019_1_OR_NEWER
                    AmbientSkiesPipelineUtils.SetupPipeline(m_pipelineToInstall, "Procedural Worlds HDRPRenderPipelineAsset", "Procedural Worlds/Simple Water HD", "Pipeline Terrain Material", "HDRP/TerrainLit", m_finalizeEnvironment);
#else
                    AmbientSkiesPipelineUtils.SetupPipeline(m_pipelineToInstall, "Procedural Worlds HDRPRenderPipelineAsset", "Procedural Worlds/Simple Water 2019 HD", "Pipeline Terrain Material", "HDRP/TerrainLit", m_finalizeEnvironment);
#endif
                    if (m_showDebug)
                    {
                        Debug.Log(m_profiles);
